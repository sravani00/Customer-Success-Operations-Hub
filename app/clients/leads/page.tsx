...
import { UserPlus, Building, Mail, Phone, ChevronRight, ArrowLeft, Plus, Edit, Trash2 } from 'lucide-react';
      ...
      {leadClients.length === 0 ? (
        <div className="bg-white border border-slate-200/90 rounded-2xl p-12 text-center shadow-xs flex flex-col items-center justify-center space-y-4">
          ...
          <div className="max-w-md space-y-1.5">
            <h3 className="text-base font-bold text-slate-900">No Lead accounts yet</h3>
            <p className="text-xs text-slate-500 leading-relaxed">
              Add your first Lead account to start tracking lead details, pipeline status, activities, and notes.
            </p>
          </div>
          <button
            onClick={() => setIsAddModalOpen(true)}
            className="px-4 py-2 rounded-xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold flex items-center space-x-1.5 shadow-xs transition-all"
          >
            <Plus className="w-4 h-4" />
            <span>Add Lead Account</span>
          </button>
