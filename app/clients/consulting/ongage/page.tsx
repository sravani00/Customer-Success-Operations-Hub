...
import { Briefcase, Building, Mail, Phone, ChevronRight, ArrowLeft, Plus, Edit, Trash2 } from 'lucide-react';
import { useAppStore } from '../../../../lib/store';
import { ClientModal } from '../../../../components/modals/client-modal';
import { Client } from '../../../../types';

export default function OngageConsultingPage() {
  const { clients, addClient, updateClient, deleteClient } = useAppStore();
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  const [editingClient, setEditingClient] = useState<Client | null>(null);

  const ongageClients = clients.filter((c) => 
    c.subModule === 'Consulting' && c.subModuleCategory === 'Ongage'
  );
    ...
    <div className="space-y-6 max-w-7xl mx-auto animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/90 shadow-xs">
          ...
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span className="text-xs px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 font-mono font-bold">
                {ongageClients.length} Accounts
              </span>
            </h1>
            <p className="text-xs text-slate-500 mt-0.5">
              Ongage ESP email marketing platform integrations, deliverability monitoring, and API setup
            </p>
          </div>
        </div>
        ...
        <button
          ...
          className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold flex items-center space-x-1.5 shadow-xs"
        >
      ...
      {ongageClients.length === 0 ? (
        <div className="bg-white border border-slate-200/90 rounded-2xl p-12 text-center shadow-xs flex flex-col items-center justify-center space-y-4">
          ...
          <div className="max-w-md space-y-1.5">
            <h3 className="text-base font-bold text-slate-900">No Ongage accounts yet</h3>
            <p className="text-xs text-slate-500 leading-relaxed">
              Add your first Ongage account to start tracking account details, revenue, feeds, agreements and activity.
            </p>
          </div>
          <button
            onClick={() => setIsAddModalOpen(true)}
            className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold flex items-center space-x-1.5 shadow-xs transition-all"
          >
            <Plus className="w-4 h-4" />
            <span>Add Account</span>
          </button>
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 space-y-2 text-xs text-slate-700">
                    <div className="flex items-center justify-between">
                      <span className="text-slate-500">Ongage Integration Specialist:</span>
