...
export default function ResoluteAffiliateNetworksPage() {
  ...
  const resoluteClients = clients.filter((c) => 
    ...
    (c.subModuleCategory === 'Resolute' || !c.subModuleCategory)
  );
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Dedicated Resolute affiliate channels, automated payout streams, and traffic feeds
            </p>
          </div>
        </div>
      ...
      {resoluteClients.length === 0 ? (
        <div className="bg-white border border-slate-200/90 rounded-2xl p-12 text-center shadow-xs flex flex-col items-center justify-center space-y-4">
          ...
          <div className="max-w-md space-y-1.5">
            <h3 className="text-base font-bold text-slate-900">No Resolute accounts yet</h3>
            <p className="text-xs text-slate-500 leading-relaxed">
              Add your first Resolute account to start tracking account details, revenue, feeds, agreements and activity.
            </p>
          </div>
          <button
            onClick={() => setIsAddModalOpen(true)}
            className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold flex items-center space-x-1.5 shadow-xs transition-all"
          >
            <Plus className="w-4 h-4" />
            <span>Add Account</span>
          </button>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div className="p-2.5 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-between">
                      ...
                      <span className="font-bold text-blue-700 font-mono">Resolute</span>
                    </div>
                    <div className="p-2.5 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-between">
